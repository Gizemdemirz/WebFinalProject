	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(page.equals("sign-up-form")) {
			...
			if(password_1.equals(password_2)) {
				...
				request.setAttribute("msg", "Hesap baþarýyla oluþturuldu, Lütfen Giriþ Yapýn!");
				request.getRequestDispatcher("login.jsp").forward(request, response);
				
			}else {
				request.setAttribute("msg", "Parolalar eþleþmiyor");
		...
		if(page.equals("login-form")) {
			...
			if(status) {
				...
				request.setAttribute("msg", "Geçersiz Kimlik Bilgileri");
				request.setAttribute("username", username);
				request.getRequestDispatcher("login.jsp").forward(request, response);
			}
		...
		if(page.equals("addtocart")) {
			...
			if(check)
				JOptionPane.showMessageDialog(null, "Ürün zaten sepete eklendi", "Bilgi", JOptionPane.INFORMATION_MESSAGE);
			else {
			cartlist.add(id);
			JOptionPane.showMessageDialog(null, "Ürün baþarýyla sepete eklendi", "Bilgi", JOptionPane.INFORMATION_MESSAGE);
			}
